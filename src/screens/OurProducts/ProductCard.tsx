...
const ProductCard: React.FC<Props> = ({ product, liked, onToggleLike }) => {
  ...
  const selectedImage = Array.isArray(product.image)
    ? getImageSource(product.image[0])
    : getImageSource(product.image);
  return (
    <SafeAreaView style={[styles.productCard, { backgroundColor: '#fff' }]}>
      <TouchableOpacity
      ...
      >
        ...
        />
        <Text numberOfLines={1} style={[styles.productName, { color: '#000' }]}>
          {product.name}
        </Text>
        <View style={styles.OurterPriceContainer}>
          <View style={styles.InnerPriceContainer}>
            ...
            <Text style={[styles.oldPriceStyle, { color: '#ccc' }]}>
              {product.oldPrice}
            </Text>
          </View>
          <Text
