...
const WalletAddMoneyScreen = ({ navigation, route }) => {
  ...
  const verifyPayment = async (razorpayOrderId, razorpayPaymentId, razorpaySignature) => {
    try {
      ...
      if (data.success || data.msg === 'success') {
        ...
        throw new Error(data.error || data.msg || 'Payment verification failed');
      }
    } catch (error) {
  ...
  const processRefund = async (paymentId) => {
    try {
      ...
      const response = await fetch('https://naushad.onrender.com/api/razorpay/refund', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  const initiatePayment = async () => {
    ...
    try {
      ...
      navigation.navigate('WalletPaymentSuccessScreen', {
        amount: amount,
      ...
      if (error.error) {
        ...
        if (error.message === 'success') {
          errorMessage = 'Payment completed but there was an issue updating your wallet. Please contact support.';
        } else {
  ...
  const WalletIcon = ({ size = 24, color = '#333' }) => (
    ...
    >
      <Text style={{ fontSize: size * 0.6, color: '#fff', fontWeight: 'bold' }}>₹</Text>
    </View>
  );
        ...
        >
          ...
          <View style={styles.currentBalanceContainer}>
            ...
            {balanceLoading ? (
              ...
              <Text style={styles.currentBalanceAmount}>₹ {walletBalance.toLocaleString()}</Text>
            )}
          </View>
          ...
          <View style={styles.benefitsContainer}>
            <Text style={styles.benefitsTitle}>Wallet Benefits</Text>
            <View style={styles.benefitItem}>
              <WalletIcon size={wp('4.5%')} color={COLORS.primary} />
              <Text style={styles.benefitText}>Instant refunds to wallet</Text>
            </View>
            <View style={styles.benefitItem}>
              <WalletIcon size={wp('4.5%')} color={COLORS.primary} />
              <Text style={styles.benefitText}>Quick checkout for services</Text>
            </View>
            <View style={styles.benefitItem}>
              <WalletIcon size={wp('4.5%')} color={COLORS.primary} />
              <Text style={styles.benefitText}>Special offers for wallet users</Text>
            </View>
          </View>
          
          {/* Terms */}
          <Text style={styles.termsText}>
            By proceeding, you agree to our Terms & Conditions. Wallet balance is non-refundable and non-transferable.
          </Text>
        ...
        <View style={styles.footer}>
          ...
          <TouchableOpacity
            style={[
              ...
              (!selectedAmount || selectedAmount < 49 || loading) && styles.proceedButtonDisabled
            ]}
...
const styles = StyleSheet.create({
  ...
  // Reduced Current Balance Container
  ...
  // Proper Custom Amount Input Height
...
export default WalletAddMoneyScreen;
